...
def get_usage():
    with sqlite3.connect(DATABASE) as conn:
        ...
        print(f"get_usage result: {result}")  # Debug statement
        return result
...
def get_access_status():
    ...
    print("Usage data is incomplete or not found.")  # Debug statement
    return None
...
def upload():
    try:
        ...
        if epub_file and allowed_file(epub_file.filename):
            secure_filename = f"{uuid.uuid4()}.epub"
            temp_epub_path = os.path.join(app.config['UPLOAD_FOLDER'], secure_filename)
            epub_file.save(temp_epub_path)

            folder_name = os.path.splitext(secure_filename)[0]
            current_folder = os.path.join(app.config['UNZIPPED_DIR'], folder_name)
            os.makedirs(current_folder, exist_ok=True)

            if extract_epub(temp_epub_path, current_folder):
                if create_html_from_epub(temp_epub_path, os.path.join(current_folder, 'file.html')):
                    return redirect(url_for('read_epub', folder_name=folder_name))
                else:
                    return jsonify({'error': 'Error creating HTML from EPUB'}), 500
            else:
                return jsonify({'error': 'Error extracting EPUB contents'}), 500
